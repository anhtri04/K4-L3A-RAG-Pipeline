# Forward all arguments to Task 1's legal document downloader.
...
foreach ($candidate in $candidates) {
    ...
    try {
        & $candidate.Executable @prefix -m src.task1_collect_legal_docs @args
...
Write-Error 'Python 3.10+ was not found. Install Python, then run: python -m src.task1_collect_legal_docs'
exit 1
